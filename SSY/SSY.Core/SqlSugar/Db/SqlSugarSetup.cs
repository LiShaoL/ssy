...
using SSY.Core.Utils;

namespace SSY.Core;

public static class SqlSugarSetup
{
    public static readonly List<SqlSugarSetting> DbConfigs = App.GetConfig<List<SqlSugarSetting>>("SqlSugarSettings:ConnectionStrings");
    ...
    public static void AddSqlSugar(this IServiceCollection services)
    {
        ...
        // 自定义 SqlSugar 雪花ID算法，默认与 YitIdHelper 保持一致。
        SnowFlakeSingle.WorkId = 1;
        StaticConfig.CustomSnowFlakeFunc = YitIdHelper.NextId;

        SqlSugarScope sqlSugar = new SqlSugarScope(DbConfigs.Adapt<List<ConnectionConfig>>(), db =>
        {
            //遍历配置的数据库
        ...
        services.AddScoped(typeof(DbRepository<>)); // 仓储注册
    ...
    private static void ExternalServicesSetting(SqlSugarScopeProvider db, SqlSugarSetting config)
    {
